...
const char* mqtt_server = "mqtt02.thedevranch.net";
const char* doorjamb_topic = "beavdoorjamb";
const char* subscriber_name = "lightsensor003";
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    // Attempt to connect
    if (client.connect(subscriber_name)) {
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (lightValue > SENSOR_THRESHOLD_FOR_FLASHING) {
   flashLED();
  }
}
